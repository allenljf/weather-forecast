...
class CityRepositoryImplTest {
    ...
    private val appLanguageDataSource = mockk<AppLanguageDataSource> {
        every { language } returns flowOf(AppLanguage.ENGLISH)
    }
    ...
    fun `searchCities maps results to domain cities`() = runTest {
        coEvery { geocodingApi.searchCities(name = "tok", language = "en") } returns GeocodingResponseDto(
            results = listOf(
    ...
    fun `searchCities returns empty list when results are null`() = runTest {
        coEvery { geocodingApi.searchCities(name = "nomatch", language = "en") } returns GeocodingResponseDto(results = null)
    ...
    fun `searchCities maps IOException to network error`() = runTest {
        coEvery { geocodingApi.searchCities(name = any(), language = any()) } throws IOException("offline")
